...
const McRoulette = ({
    ...
    const [isShowBigWinnerImage, setIsShowBigWinnerImage] = useState<boolean>(false)
    ...
    return (
        <div>
            ...
            {isShowBigWinnerImage &&
                <div  className={cl.bigPicture}>
                    <img
                        src={rouletteWeapons.filter((w, i) => i == weaponPrizeId)[0].steam_image}
                        alt="Большая картинка"
                    />
                    <p className={cl.bigPictureText}>Конгретулэйшон!!!!</p>
                    <button onClick={() => setIsShowBigWinnerImage(false)} className={cl.bigPictureButton}>Убрать</button>
                </div>
